...
# Initialize session state
if "messages" not in st.session_state:
    st.session_state.messages = [("assistant", "How can I help with medical info today?")]
if "chat_history" not in st.session_state:
    st.session_state.chat_history = []
if "symptoms_input" not in st.session_state:
    st.session_state.symptoms_input = ""
    ...
    if user_input:
        ...
        try:
            with st.spinner("Processing..."):
                response = model_api.get_response(user_input, st.session_state.chat_history)
...
symptoms_input = st.text_area("Enter symptoms (e.g., fever, cough):", key="symptoms_input")
if st.button("Check Symptoms"):
    if symptoms_input:
        try:
            with st.spinner("Analyzing symptoms..."):
